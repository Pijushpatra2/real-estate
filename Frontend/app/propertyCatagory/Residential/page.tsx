...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        setIsLoading(true);
        const res = await api.get("/properties/get/all?prop_type=residential"); // 🔍 Residential filter

        if (Array.isArray(res.data)) {
          const transformedProjects = res.data
            .filter((p: any) => p.prop_type?.toLowerCase() === "residential")
            .map((project: any) => ({
    ...
    <div className="pt-20">
      <div className="bg-gray-100">
        <div className="container mx-auto px-4 py-12 md:py-20">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Residential Projects</h1>
    ...
    <div className="pt-20">
      <div className="bg-gray-100">
        <div className="container mx-auto px-4 py-12 md:py-20">
          ...
          <p className="text-xl text-gray-600 max-w-3xl">
            Explore our elegant residential developments crafted for modern
            living.
          </p>
        </div>
      </div>
            ...
            >
              <div className="relative h-64">
                ...
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center">
                  <Home className="h-4 w-4 text-lime-500 mr-1" />
